...
class CreateVenteg extends Component
{
    ...
    {
        // Déboguer pour voir ce qui est reçu
        // \Log::info('updatedVgs', ['key' => $key, 'value' => $value]);

        // Recherche par code barre - pour n'importe quelle ligne
       if (str_contains($key, 'code_barre')) {
            // Extraire l'index du tableau
            $parts = explode('.', $key);
            // La structure est: "1.code_barre" ou "0.code_barre"
            $index = (int)($parts[0] ?? -1);

            // \Log::info('Recherche pour index: ' . $index . ' avec code: ' . $value);

            if ($index >= 0 && isset($this->vgs[$index])) {
