...
class Listedetails extends Component
{
    use WithPagination;
    public $search = '';
   use WithDelete {
        ...
        $listedetail=Listedetail::where('id','=',$this->deleteId)->first();
         ...
         if ($ar_principal and $ar_secondaire) {
            ...
            if ($ar_secondaire->ar_qte<$listedetail->lds_qte_ds) {
                return redirect()->route('listedetails.index')->with('danger', __('cette opération ne peut pas être executée car la quantité de détail est supérieur à la quantité réel'));
            } else {
                ...
                trace($data,$model);
                return redirect()->route('listedetails.index')->with('success', __("L'opération de détail article est supprimée"));
            }
         } else {
            return redirect()->route('listedetails.index')->with('danger', 'il y a une erreur');
         }
    ...
    { 
        ...
        return view('livewire.listedetails',compact('listedetails'));
    }
}
