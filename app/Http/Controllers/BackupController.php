...
class BackupController extends Controller
{
    ...
    /**
     * Détection du chemin mysqldump (Windows/Linux) - Version améliorée
     */
    private function findMysqldump()
    {
        ...
        foreach ($linuxPaths as $path) {
            if (file_exists($path)) {
                \Log::info('mysqldump found (Linux): ' . $path);
                return $path;
            }
        }
        ...
        foreach ($wampPaths as $path) {
            if (file_exists($path)) {
                \Log::info('mysqldump found (WAMP): ' . $path);
                return $path;
            }
        }
        ...
        $xamppPaths = [
            ...
            'C:\\xampp74\\mysql\\bin\\mysqldump.exe',
            'C:\\xampp80\\mysql\\bin\\mysqldump.exe',
        ];
        
        foreach ($xamppPaths as $path) {
            if (file_exists($path)) {
                \Log::info('mysqldump found (XAMPP): ' . $path);
                return $path;
            }
        }
        
        // 4. PHP Desktop - Recherche dans le dossier d'installation
        $phpDesktopPaths = [
            // Depuis le dossier de l'application
            realpath(base_path('../mariadb/bin/mysqldump.exe')),
            realpath(base_path('../../mariadb/bin/mysqldump.exe')),
            realpath(base_path('../../../mariadb/bin/mysqldump.exe')),
            realpath(dirname(base_path()) . '/mariadb/bin/mysqldump.exe'),
            realpath(getcwd() . '/mariadb/bin/mysqldump.exe'),
            realpath(getcwd() . '/../mariadb/bin/mysqldump.exe'),
            realpath(getcwd() . '/../../mariadb/bin/mysqldump.exe'),
            // Chemins absolus possibles
            'C:\\phpdesktop\\mariadb\\bin\\mysqldump.exe',
            'C:\\Program Files\\phpdesktop\\mariadb\\bin\\mysqldump.exe',
            'D:\\phpdesktop\\mariadb\\bin\\mysqldump.exe',
        ];
        
        foreach ($phpDesktopPaths as $path) {
            if ($path && file_exists($path)) {
                \Log::info('mysqldump found (PHP Desktop): ' . $path);
                return $path;
            }
        }
        
        // 5. Recherche récursive dans le dossier parent (plus lent mais plus complet)
        $searchPaths = [
            base_path(),
            dirname(base_path()),
            dirname(base_path(), 2),
        ...
        foreach ($searchPaths as $searchPath) {
            if (is_dir($searchPath)) {
                $found = $this->searchMysqldumpRecursive($searchPath);
                if ($found) {
                    \Log::info('mysqldump found (recursive search): ' . $found);
                    return $found;
                }
            }
        }
        
        // 6. Tester dans le PATH système
        $test = @shell_exec('where mysqldump 2>nul');
        if (!$test) {
            $test = @shell_exec('which mysqldump 2>/dev/null');
        }
        if ($test && trim($test)) {
            \Log::info('mysqldump found (PATH): ' . trim($test));
            return trim($test);
        }
        
        \Log::warning('mysqldump not found in any location');
        return null;
    }
    
    /**
     * Recherche récursive de mysqldump.exe dans un dossier
     */
    private function searchMysqldumpRecursive($directory, $maxDepth = 3)
    {
        if ($maxDepth <= 0) {
            return null;
        }
        ...
        try {
            ...
            foreach ($iterator as $file) {
                if ($file->isFile() && strtolower($file->getFilename()) === 'mysqldump.exe') {
    ...
    /**
     * Détection du chemin mysql (Windows/Linux) - Version améliorée
     */
    private function findMysql()
    {
        ...
        // 4. PHP Desktop
        $phpDesktopPaths = [
            realpath(base_path('../mariadb/bin/mysql.exe')),
            realpath(base_path('../../mariadb/bin/mysql.exe')),
            realpath(getcwd() . '/mariadb/bin/mysql.exe'),
            'C:\\phpdesktop\\mariadb\\bin\\mysql.exe',
        ];
    ...
    /**
     * Route de diagnostic - Pour tester les chemins
     */
    public function diagnose()
    {
        ...
        $result = [
            ...
            'mysql_path' => $mysql,
            'can_use_mysqldump' => $this->canUseMysqldump(),
