    ...
    <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-4">
        ...
        <div class="flex justify-between items-center mb-4">
            ...
            @include('partials.bouton_ajouter', [
                'url' => route('methodes.create'),
                'texte' => __('Ajouter une méthode de paiement'),
                'couleur' => 'blue',
                'icone' => true,
            ])
            {{-- @endcan --}}
                        ...
                        <table class="min-w-full text-center">
                            ...
                            <thead class="border-b bg-gray-50">
                                <tr style="vertical-align:middle">
                                    ...
                                    <th class="text-sm font-medium text-gray-900 px-2 py-2">{{ __('Téléphone') }}</th>
                                    <th class="text-sm font-medium text-gray-900 px-2 py-2">{{ __('Active') }}</th>
                                ...
                                @forelse ($methodes as $item)
                                    <tr class="border-b-2 border-gray-100 hover:bg-gray-50" style="vertical-align:middle">
                                        ...
                                        <td class="text-sm font-medium text-gray-900 px-2 py-2">
                                            <span class="number-cell">{!! format_number($item->meth_tel) !!}</span>
                                        </td>
                                        <td class="text-sm font-medium text-gray-900 px-2 py-2">
                                            @if ($item->meth_active)
                                                <span class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs">{{ __('Oui') }}</span>
                                            @else
                                                <span class="px-2 py-1 bg-red-100 text-red-800 rounded-full text-xs">{{ __('Non') }}</span>
                                            @endif
                                        </td>
                                        <td class="text-sm font-medium text-gray-900 px-2 py-2">
                                            ...
                                            <div class="flex justify-center space-x-1 rtl:space-x-reverse">
                                                @can('update methode')
                                                    @include('partials.boutton_modifier', [
                                                        'url' => route('methodes.edit', $item->id)
                                                    ])
                                                @endcan
