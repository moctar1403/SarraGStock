        ...
        @can('create article')
            <div class="flex flex-wrap gap-2 p-2">
                @include('partials.bouton_ajouter', [
                    ...
                    'texte' => __('Effectuer une opération de détail'),
                    'couleur' => 'blue',
                    'icone' => false,
                ])
            </div>
                    ...
                    <table class="min-w-full text-center">
                        ...
                        <thead class="border-b bg-gray-50">
                            <tr style="vertical-align:middle">
                                ...
                                <th class="text-sm font-medium text-gray-900 px-2 py-2">{{ __('Qté principale') }}</th>
                                <th class="text-sm font-medium text-gray-900 px-2 py-2">{{ __('Qté détaillée/unité') }}</th>
                                <th class="text-sm font-medium text-gray-900 px-2 py-2">{{ __('Qté totale détaillée') }}</th>
                        ...
                        {{-- Corps du tableau --}}
                        <tbody>
                            @forelse ($listedetails as $item)
                                @php
                                    $articlePrincipal = App\Models\Article::find($item->lds_principal);
                                    $articleDetail = App\Models\Article::find($item->lds_detail);
                                @endphp
                                
                                <tr class="border-b-2 border-gray-100 hover:bg-gray-50" style="vertical-align:middle">
                                    <td class="text-sm font-medium text-gray-900 px-2 py-2">{{ $item->created_at->format('d/m/Y') }}</td>
                                    <td class="text-sm font-medium text-gray-900 px-2 py-2">{{ $item->id }}</td>
                                    <td class="text-sm font-medium text-gray-900 px-2 py-2">{{ $articlePrincipal->ar_lib ?? __('Non défini') }}</td>
                                    <td class="text-sm font-medium text-gray-900 px-2 py-2">{{ $articleDetail->ar_lib ?? __('Non défini') }}</td>
                                    <td class="text-sm font-medium text-gray-900 px-2 py-2">
                                        <span class="number-cell">{!! format_number($item->lds_qte_pr) !!}</span>
                                    </td>
                                    <td class="text-sm font-medium text-gray-900 px-2 py-2">
                                        <span class="number-cell">{!! format_number($item->lds_qte_pr_par_ds) !!}</span>
                                    </td>
                                    <td class="text-sm font-medium text-gray-900 px-2 py-2">
                                        <span class="number-cell">{!! format_number($item->lds_qte_ds) !!}</span>
                                    </td>
                                    <td class="text-sm font-medium text-center whitespace-nowrap">
                                        <div class="flex justify-center space-x-1 rtl:space-x-reverse">
                                            @can('delete listedetails')
