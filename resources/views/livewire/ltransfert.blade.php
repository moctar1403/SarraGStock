        ...
        <div class="flex justify-between items-center mb-4">
            ...
            @can('create transfert')
                @include('partials.bouton_ajouter', [
                    'url' => route('operation.methodes'),
                    'texte' => __('Ajouter un transfert'),
                    'couleur' => 'blue',
                    'icone' => true,
                ])
            @endcan
                        ...
                        <table class="min-w-full text-center">
                            ...
                            <thead class="border-b bg-gray-50">
                                <tr style="vertical-align:middle">
                                    <th class="text-sm font-medium text-gray-900 px-2 py-2">{{ __('Méthode Envoyante') }}</th>
                                    <th class="text-sm font-medium text-gray-900 px-2 py-2">{{ __('Méthode Bénéficiante') }}</th>
                            ...
                            {{-- Corps du tableau --}}
                            <tbody>
                                @forelse ($transfert as $item)
                                    <tr class="border-b-2 border-gray-100 hover:bg-gray-50" style="vertical-align:middle">
                                        <td class="text-sm font-medium text-gray-900 px-2 py-2">{{ $item->tr_meth_env_nom }}</td>
                                        <td class="text-sm font-medium text-gray-900 px-2 py-2">{{ $item->tr_meth_ben_nom }}</td>
                                        <td class="text-sm font-medium text-gray-900 px-2 py-2">
                                            <span class="number-cell">{!! format_number($item->tr_montant) !!}</span>
                                        </td>
                                        <td class="text-sm font-medium text-gray-900 px-2 py-2">
                                            <span class="number-cell">{!! format_number($item->tr_frais) !!}</span>
                                        </td>
